...
class C_knowledge_publish extends CI_Controller {
	...
	public function ajax_list(){
		if ($this->ion_auth->logged_in()){
			$list = $this->m_knowledge->get_datatables($this->id_user, 2); // view untuk table publish
			...
			foreach ($list as $knowledge) {				
				...
				if($knowledge->status_knowledge == 2){ 
	...
	public function ajax_publish($id_knowledge){
		if ($this->ion_auth->logged_in()){
			$this->m_knowledge->publish_($id_knowledge);
			log_act("validasi", "validasi data knowledge (#$id_knowledge)");
			echo json_encode(array("status" => TRUE));
		}
	}
}